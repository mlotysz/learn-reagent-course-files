  ...
  [:div.form__group
   ...
   [:input.form__input
    {:type type
     ...
     :value value
     :on-change
     #(swap! values assoc (keyword id) (.. % -target -value))}]])
  ...
  (let [{:keys [title desc img price sold-out]} @values]
    [:div.modal (when (:active @modal) {:class "active"})
     ...
     [:div.modal__container
      [:div.modal__body
       [form-group {:id "title" :type "text" :value title :values values}]
       [form-group {:id "desc" :type "text" :value desc :values values}]
       [form-group {:id "img" :type "text" :value img :values values}]
       [form-group {:id "price" :type "number" :value price :values values}]
       [:div.form__group
